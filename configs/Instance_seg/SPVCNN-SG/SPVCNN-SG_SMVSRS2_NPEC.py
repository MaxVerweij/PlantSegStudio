...
DATA_MODE = 'inference'
assert DATA_MODE in ['inference','training'], "Please select a valid training mode"
npec_data_len = 59
if DATA_MODE=='training':
    # The data contains files from NPEC and Soybean-MVSRS
    # NPEC data is 1 ... 59, so 59 files long. Soybean-MVSRS is 60 ... 161
    # I want to train (and validate) with Soybean-MVSRS data only, so 
    train_area = list(range(1+npec_data_len, 45+npec_data_len)) + list(range(65+npec_data_len, 86+npec_data_len))
    test_area = list(range(45+npec_data_len, 65+npec_data_len)) + list(range(86+npec_data_len, 103+npec_data_len))
if DATA_MODE=='inference':
    train_area = []
    test_area = list(range(1,npec_data_len+1))
