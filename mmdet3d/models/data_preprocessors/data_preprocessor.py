...
from ..ops.softgroup_ops import voxelization_idx
...
class Det3DDataPreprocessor(DetDataPreprocessor):
        ...
        if 'img' in data['inputs']:
            ...
            if is_seq_of(_batch_imgs, torch.Tensor):
                ...
                if img_dim == 3:
                    ...
                    batch_imgs = multiview_img_stack_batch(batch_imgs, self.pad_size_divisor,
                                                           self.pad_value)
            ...
            # Process data with `default_collate`.
                ...
                pad_h = target_h - h
                pad_w = target_w - w
                batch_imgs = F.pad(_batch_imgs, (0, pad_w, 0, pad_h), 'constant', self.pad_value)
            else:
    ...
    def _get_pad_shape(self, data: dict) -> List[Tuple[int, int]]:
        ...
        if is_seq_of(_batch_inputs, torch.Tensor):
            ...
            for ori_input in _batch_inputs:
                ...
                pad_h = int(np.ceil(
                    ori_input.shape[1] / self.pad_size_divisor)) * self.pad_size_divisor
                pad_w = int(np.ceil(
                    ori_input.shape[2] / self.pad_size_divisor)) * self.pad_size_divisor
        ...
        # Process data with `default_collate`.
            ...
            pad_h = int(np.ceil(
                _batch_inputs.shape[1] / self.pad_size_divisor)) * self.pad_size_divisor
            pad_w = int(np.ceil(
                _batch_inputs.shape[2] / self.pad_size_divisor)) * self.pad_size_divisor
    ...
    def voxelize(self, points: List[Tensor], data_samples: SampleList) -> Dict[str, Tensor]:
        ...
        if self.voxel_type == 'hard':
            ...
            coors = torch.cat(coors, dim=0)
        elif self.voxel_type == 'dynamic_quantize':
            voxel, coors = [], []
            voxel_size = points[0].new_tensor(self.voxel_layer.voxel_size)
            # dynamic voxelization only provide a coors mapping (coors)
            ...
            voxels = torch.cat(voxel, dim=0)
            coors = torch.cat(coors, dim=0)
        elif self.voxel_type == 'dynamic_quantize2':
            coors = []
            voxel_size = points[0].new_tensor(self.voxel_layer.voxel_size)
            # dynamic voxelization only provide a coors mapping (coors)
            for i, (res, data_sample) in enumerate(zip(points, data_samples)):
                ...
                try:  # only support PyTorch >= 1.9.0
                    ...
                    for coor_idx in range(3):
                        polar_res_clamp[:, coor_idx][
                            polar_res[:, coor_idx] > max_bound[coor_idx]] = max_bound[coor_idx]
                        polar_res_clamp[:, coor_idx][
                            polar_res[:, coor_idx] < min_bound[coor_idx]] = min_bound[coor_idx]
                res_coors = torch.floor(
                    (polar_res_clamp - min_bound) /
                    polar_res_clamp.new_tensor(self.voxel_layer.voxel_size)).int()
            ...
            voxels = torch.cat(voxels, dim=0)
            coors = torch.cat(coors, dim=0)
        elif self.voxel_type == 'dynamic_minkunet':
            dynamic_voxels, dynamic_coors = [], []
            voxel_inds, coors = [], []
            voxel_size = points[0].new_tensor(self.voxel_layer.voxel_size)
            for i, (res, data_sample) in enumerate(zip(points, data_samples)):
                dynamic_coors = self.voxel_layer(res)
                dynamic_coors = F.pad(dynamic_coors, (1, 0), mode='constant', value=i)
                dynamic_coors.append(dynamic_coors)
                res_coors = torch.round(res[:, :3] / voxel_size).int()
                res_coors -= res_coors.min(0)[0]
                ...
                res_voxels = res[inds]
                if self.batch_first:
                    res_voxel_coors = F.pad(res_voxel_coors, (1, 0), mode='constant', value=i)
                    data_sample.batch_idx = res_voxel_coors[:, 0]
                else:
                    res_voxel_coors = F.pad(res_voxel_coors, (0, 1), mode='constant', value=i)
                    data_sample.batch_idx = res_voxel_coors[:, -1]
                data_sample.point2voxel_map = point2voxel_map.long()
            dynamic_voxels = torch.cat(points, dim=0)
            dynamic_coors = torch.cat(dynamic_coors, dim=0)
    ...
    def get_voxel_seg(self, res_coors: Tensor, data_sample: SampleList) -> None:
        ...
        if self.training:
            ...
            _, _, point2voxel_map = dynamic_scatter_3d(pseudo_tensor, res_coors, 'mean', True)
            data_sample.point2voxel_map = point2voxel_map
...
@MODELS.register_module()
class SoftGroupDataPreprocessor(Det3DDataPreprocessor):
    """Preprocessor for softgroup.
    ...
    - pt_offset_labels (torch.Tensor)

    Args:
        voxel_cfg (dict): Config for voxelization layer.
    """

    def __init__(self, voxel_cfg: dict, **kwargs) -> None:
    ...
    def simple_process(self, data: dict, training: bool = False) -> dict:
        """Process point cloud data for one batch.
        ...
        if 'points' in inputs:
            ...
            for i, sample in enumerate(data_samples):
                ...
                inst_info = sample.gt_instances_softgroup
                inst_num = inst_info['instance_num']
                inst_pointnum = inst_info['instance_pointnum']
                inst_cls = inst_info['instance_cls']
                pt_offset_label = inst_info['pt_offset_labels']
                pt_offset_label = torch.from_numpy(pt_offset_label)
                if self.training:
                    pts_inst_mask = sample.gt_pts_seg.pts_instance_mask
                    pts_semantic_mask = sample.gt_pts_seg.pts_semantic_mask
                else:
                    pts_inst_mask = torch.from_numpy(sample.eval_ann_info['pts_instance_mask'])
                    pts_semantic_mask = torch.from_numpy(sample.eval_ann_info['pts_semantic_mask'])
                # Update instance labels

                instance_label = pts_inst_mask
                instance_label[instance_label != -100] += total_inst_num
                total_inst_num += inst_num
                # Gather batch data
                scan_ids.append(osp.basename(sample.lidar_path).replace('.bin', ''))
                # after all data aug, xyz = xyz_middle * self.voxel_cfg.scale
                cur_coords = points[:, :3] * self.voxel_cfg.scale
                cur_coords = cur_coords - cur_coords.min(0)[0]
                cur_coords = cur_coords.long()
                cur_coords = torch.cat(
                    [cur_coords.new_full((cur_coords.shape[0], 1), batch_id), cur_coords], 1)
                coords.append(cur_coords)
                coords_float.append(points[:, :3])
                feats.append(points)  # Use all input features as feats
                semantic_labels.append(pts_semantic_mask)
            ...
            spatial_shape = np.clip(
                coords.max(0)[0][1:].cpu().numpy() + 1, self.voxel_cfg.spatial_shape[0], None)

            voxel_coords, v2p_map, p2v_map = voxelization_idx(coords.cpu(), batch_id)
            voxel_coords, v2p_map, p2v_map, pt_offset_labels, \
                instance_cls, instance_pointnum = [
                    tensor.to(coords.device) for tensor in [
                        voxel_coords, v2p_map, p2v_map, pt_offset_labels, instance_cls, instance_pointnum]]
            # Pack processed data
            batch_inputs.update({
                'scan_ids': scan_ids,
                'coords': coords,
