...
def parse_args():
    ...
    parser.add_argument('--save-local', action='store_true', help='save prediction results locally')
    parser.add_argument(
        '--show-dir',
    ...
    parser.add_argument(
        ...
        type=str,
        choices=[
            'mono_det', 'multi-view_det', 'lidar_det', 'lidar_seg', 'lidar_inst_seg',
            'multi-modality_det'
        ],
...
def trigger_inference_hook(cfg, args):
    if not hasattr(cfg, 'custom_hooks') or cfg.custom_hooks is None:
        cfg.custom_hooks = []
    custom_hooks = cfg.custom_hooks
    all_task_choices = ['lidar_det', 'lidar_seg', 'lidar_inst_seg']
    assert args.task in all_task_choices, 'You must set '\
        f"'--task' in {all_task_choices} in the command " \
        'if you want to use visualization hook'
    custom_hooks.append(
        dict(
            type='Det3DInferenceHook',
            vis_task=args.task,
            save_dir=osp.join(cfg.work_dir, f'local_inference_{args.task}')))
    return cfg
